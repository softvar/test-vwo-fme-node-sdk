 ...
 */
import { getDebugEventSamplingPercent, getUsageStatsSamplingPercent, passesSamplingPercent, shouldApplyEventSampling, wasSdkInitializedEarlier, } from '../utils/InternalEventsRuntimeUtil.js';
/**
...
export class InternalEventsThrottleService {
     ...
     */
    constructor(_serviceContainer, randomValueProvider = Math.random) {
     ...
     */
    shouldSendSdkInitEvent(settings = {}) {
     ...
     */
    shouldSendUsageStatsEvent(settings = {}) {
     ...
     */
    shouldSendSampledDebugEvent(settings = {}) {
        // if the alwaysApplySampling flag is false then we need not to sample and send that event directly.
        if (!shouldApplyEventSampling(settings)) {
            return true;
        }
